...
O papel `founder` é único por índice parcial. Um trigger impede mudança direta de `system_role` e exclusão do Founder, inclusive em operações com Service Role. Promoções e rebaixamentos de administradores passam exclusivamente pela função atômica `set_platform_admin_role`, que valida o Founder ator e registra o antes/depois em `audit_logs`.
