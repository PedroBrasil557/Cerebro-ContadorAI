...
import { TrendingUp, TrendingDown, Wallet, ArrowUpRight, ArrowDownRight, Target, ShieldCheck, CreditCard as CardIcon, Sparkles, Plus, RefreshCw } from 'lucide-react'
...
const MetricCard = ({ title, value, subtext, trend, icon: Icon, colorTheme, delay }: any) => {
    const themes: any = {
    ...
    return (
    <PremiumCard delay={delay} glowColor={`from-${colorTheme}-500/10`} className="p-6 lg:p-8 flex flex-col justify-between min-h-[180px]">
        <div className="flex items-start justify-between mb-4">
            <div className={`p-3.5 rounded-2xl ${theme.bg} border border-white/5 shadow-lg ${theme.glow} relative overflow-hidden group-hover:scale-110 transition-transform duration-500`}>
                <div className="absolute inset-0 bg-gradient-to-br from-white/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"/>
                <Icon className={`h-7 w-7 ${theme.icon} relative z-10`} />
            </div>
            
            {trend && (
            <div className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-bold tracking-wide backdrop-blur-md border border-white/5 ${trend > 0 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
        ...
        <div>
            <h3 className="text-sm text-gray-400 font-bold uppercase tracking-widest mb-2 opacity-80">{title}</h3>
            <p className={`text-4xl lg:text-5xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-br ${theme.gradient} drop-shadow-sm`}>
  ...
  return (
    <div className="p-6 md:p-10 space-y-10 max-w-[1800px] mx-auto pb-32">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
         ...
         {/* GRÁFICO */}
         <PremiumCard className="lg:col-span-8 p-8 flex flex-col h-[550px]" delay={0.4} glowColor="from-blue-600/10">
            <div className="flex justify-between items-start mb-10">
               <div>
                  <h3 className="text-2xl font-bold text-white flex items-center gap-2">
                      Fluxo de Caixa <Sparkles className="text-blue-400 h-5 w-5 animate-pulse"/>
                  </h3>
                  <p className="text-sm text-gray-400 font-medium mt-1 tracking-wide">Análise de evolução patrimonial em tempo real.</p>
               </div>
         ...
         {/* LISTA DE ATIVIDADE RECENTE (SEM ÍCONE ACTIVITY) */}
         <PremiumCard className="lg:col-span-4 p-0 flex flex-col h-[550px]" delay={0.5} glowColor="from-violet-600/10">
            <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.01]">
               <div>
                   {/* Substituído Activity por TrendingUp */}
               ...
               {recentTransactions && recentTransactions.length > 0 ? (
                  recentTransactions.map((t: any, idx: number) => {
                     ...
                     <motion.div 
                        ...
                        key={t.id} 
                        className="flex items-center justify-between p-4 rounded-2xl hover:bg-white/5 transition-all duration-300 group border border-transparent hover:border-white/5"
                     >
                        <div className="flex items-center gap-4">
                           <div className={`p-3 rounded-xl ${iconBg} ${iconColor} ring-1 ring-inset ring-white/5 group-hover:scale-105 transition-transform`}>
                              <Icon size={18} strokeWidth={2.5} />
                           </div>
                           <div className="overflow-hidden">
                              <p className="text-sm font-bold text-white truncate w-32 group-hover:text-blue-200 transition-colors">{t.description}</p>
                              <p className="text-[11px] text-gray-500 font-bold uppercase tracking-wider mt-0.5">{t.category}</p>
                           </div>
                        </div>
                        <span className={`font-mono font-black text-sm ${amountColor} tracking-tight`}>
            ...
            <div className="p-4 border-t border-white/5 bg-white/[0.01]">
               <button 
                  ...
                  className="w-full py-3 rounded-xl bg-white/5 hover:bg-white/10 text-xs font-bold uppercase tracking-widest text-white transition-colors border border-white/5"
               >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
         <PremiumCard className="p-8" delay={0.6} glowColor="from-pink-500/10">
            <div className="flex items-center gap-3 mb-8">
               <div className="p-2.5 bg-pink-500/10 rounded-xl text-pink-400 shadow-lg shadow-pink-500/20">
                   <Target size={24} />
               </div>
               <h3 className="text-xl font-bold text-white">Objetivos Financeiros</h3>
            </div>
               ...
               {goals.length > 0 ? goals.slice(0,2).map((g: any) => {
                  ...
                  <div key={g.id} className="group">
                     <div className="flex justify-between text-sm mb-3">
                        <span className="text-white font-bold text-base">{g.title}</span>
                        <div className="text-right">
                           <span className="text-white font-black">{percentage.toFixed(0)}%</span>
                           <span className="text-gray-500 text-xs ml-2 font-medium">{formatCurrency(g.current_amount)} de {formatCurrency(g.target_amount)}</span>
                        </div>
                     </div>
                   ...
                   <div className="flex flex-col items-center justify-center py-10 border-2 border-dashed border-white/10 rounded-2xl">
                       <p className="text-gray-400 font-bold">Nenhuma meta definida.</p>
                       <button onClick={() => onNavigate('investimentos')} className="mt-4 text-pink-400 text-sm font-bold uppercase tracking-wider hover:underline">+ Criar Nova Meta</button>
                   </div>
               )}
            </div>
         </PremiumCard>
         ...
         <PremiumCard className="p-1 overflow-hidden relative group" delay={0.7}>
            ...
            <div className="bg-gradient-to-br from-blue-900/40 via-[#09090b] to-[#09090b] h-full w-full rounded-[1.9rem] p-8 flex flex-col justify-center relative z-10 overflow-hidden">
               ...
               <CardIcon size={40} className="text-blue-400 mb-6 relative z-10 drop-shadow-[0_0_15px_rgba(59,130,246,0.4)]" />
               <h3 className="text-2xl font-bold text-white max-w-xs leading-tight relative z-10">
                  Gerencie seus Cartões Inteligentes
               </h3>
               <p className="text-gray-400 mt-3 max-w-sm relative z-10 mb-8 font-medium">
                  Controle limites, faturas e vencimentos em um só lugar com análise preditiva.
               </p>
               
               <button 
                  onClick={() => onNavigate('minha carteira')}
                  className="flex items-center justify-center gap-2 w-fit bg-white text-black px-8 py-4 rounded-xl font-bold hover:bg-blue-50 hover:scale-105 transition-all active:scale-95 shadow-xl shadow-blue-500/10 relative z-10"
               >
