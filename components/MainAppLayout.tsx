...
export default function MainAppLayout({ session }: { session: Session }) {
  ...
  const fetchData = useCallback(async () => {
    try {
        setLoading(true)
        // 1. Obter usuário REAL (UUID Válido)
        ...
        if (profile) {
            ...
            // Mock local seguro enquanto não salva no banco
            setUserProfile({
                id: user.id,
                full_name: '',
                email: user.email || '',
  ...
  // --- CORREÇÃO OBRIGATÓRIA: PERFIL ---
  const handleUpdateProfile = async (data: any) => {
      // 1. Obter UUID Real agora (evita "user-123")
      const { data: { user }, error: authError } = await supabase.auth.getUser()
      
      if (authError || !user || !user.id) {
          console.error("Erro fatal de sessão:", authError)
          alert("Sessão inválida. Faça login novamente.")
          return
      }

      console.log("Salvando perfil para UUID Real:", user.id)

      // 2. Upsert com ID seguro
      const { data: savedProfile, error } = await supabase.from('profiles').upsert({
          id: user.id, // <--- OBRIGATÓRIO SER UUID REAL
      ...
      if (error) { 
          console.error("Erro Supabase Perfil:", error)
  ...
  const handleAddCard = async (card: any) => {
      ...
      if (data && !error) setCards(prev => [...prev, data])
  }
  ...
  // --- AGENDAMENTO -> FINANCEIRO (BLINDADO) ---
  ...
  // --- INSERÇÕES SIMPLES (BLINDADAS COM UUID REAL) ---
  const handleAddAppointment = async (a: any) => {
      ...
      const { data } = await supabase.from('appointments').insert({ 
          user_id: user.id, 
          client_name: a.clientName, service: a.service, value: a.value, date: a.date, time: a.time, caixa_percentage: a.caixaPercentage 
      }).select().single()
      
      if(data) setAppointments(prev => [...prev, {...data, clientName: data.client_name, caixaPercentage: data.caixa_percentage}])
  }
  ...
  const handleAddTransaction = async (t: any) => {
      ...
      const { data } = await supabase.from('transactions').insert({ 
          user_id: user.id, 
          description: t.description, amount: t.amount, type: t.type, category: t.category, date: t.date 
      }).select().single()
  ...
  const handleUpdateGoal = async (g: any) => {
      await supabase.from('goals').update({ current_amount: g.current_amount }).eq('id', g.id)
  ...
  return (
    <div className="flex min-h-screen bg-[#0a0a0a] text-white font-sans selection:bg-violet-500 selection:text-white">
      ...
      <main className="flex-1 flex flex-col md:pl-[260px] transition-all duration-300">
        <TopBar title={activeTab} user={session?.user} profile={userProfile} notifications={MOCK_NOTIFICATIONS} onToggleMenu={() => setIsMenuOpen(!isMenuOpen)} onNavigate={setActiveTab} onLogout={handleLogout} onGeneratePDF={handleGeneratePDF} />
        <div className="flex-1 overflow-x-hidden bg-[url('/bg-grid.svg')] bg-fixed">
           {loading ? <div className="flex h-full items-center justify-center"><Loader2 className="h-10 w-10 animate-spin text-violet-600" /></div> : (
               <ViewContainer
                  ...
                  charts={{ monthlyBalanceHistory: [] }} cards={cards} goals={goals} emergencyFund={{ id: '1', current_amount: caixaTotal, goal_amount: 50000 }} cdiRate={0.1165} transactions={transactions} appointments={appointments} caixaData={{ currentBalance: caixaTotal, monthlyGoal: 5000, entries: caixaEntries }}
