    ...
    <div className="mx-auto w-full max-w-[1180px] space-y-6 p-4 md:p-6 xl:p-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm leading-5 text-[var(--color-text-secondary)]">Transforme planos em objetivos financeiros acompanháveis.</p>
        </div>
      ...
      {highlightedGoal ? (
        <div className="grid gap-5 lg:grid-cols-2">
          <GoalProgress title={highlightedGoal.title} current={Number(highlightedGoal.current_amount || 0)} target={Number(highlightedGoal.target_amount || 0)} state={goalState(highlightedGoal)} helper={deadlineLabel(highlightedGoal.deadline)} />
          <CerebroAICard title="Quer entender como encaixar esta meta no seu mês?" description={`A meta “${highlightedGoal.title}” está em ${Math.round((Number(highlightedGoal.current_amount || 0) / Math.max(Number(highlightedGoal.target_amount || 1), 1)) * 100)}%. Revise suas movimentações antes de decidir qualquer ajuste.`} actionLabel="Ver transações" onAction={() => handleRedirect('transações')} />
        </div>
      ) : null}
      ...
      <section className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-xs leading-[18px] text-[var(--color-text-helper)]">Acompanhe o progresso e movimente o valor reservado de cada objetivo.</p>
        </div>
        {goals.length ? (
            ...
            {goals.map((goal) => (
              <article key={goal.id} className="overflow-hidden rounded-[var(--radius-lg)] border border-[var(--color-card-border)] bg-[var(--color-card-fill)]">
                <div className="border-b border-[var(--color-card-border)]">
                  <GoalProgress title={goal.title} current={Number(goal.current_amount || 0)} target={Number(goal.target_amount || 0)} state={goalState(goal)} helper={deadlineLabel(goal.deadline)} className="border-0" />
                </div>
          ...
          <div className="flex min-h-64 flex-col items-center justify-center rounded-[var(--radius-lg)] border border-dashed border-[var(--color-card-border)] bg-[var(--color-card-fill)] p-8 text-center">
            <span className="flex h-12 w-12 items-center justify-center rounded-full bg-[var(--color-card-accent-fill)] text-[var(--color-nav-active-text)]"><Target className="h-6 w-6" /></span>
            <h3 className="mt-4 text-base font-semibold">Você ainda não criou uma meta</h3>
            <p className="mt-2 max-w-md text-sm leading-6 text-[var(--color-text-secondary)]">Crie um objetivo real para começar a acompanhar valor reservado e prazo.</p>
