...
import { 
  ...
  Trash2, Tag, Info, Camera, Activity, Wallet, BrainCircuit, Loader2, Pencil, Image as ImageIcon, Download, RefreshCcw
} from 'lucide-react'
...
export default function SmartShoppingView() {
  ...
  // ============================================================================
  // 🕹️ HANDLERS GERAIS & NOVAS FUNÇÕES
  // ============================================================================
  
  // 🔄 DESMARCAR TODOS OS ITENS (Zerar progresso mantendo a lista)
  const handleUncheckAll = async () => {
    if (!window.confirm("Deseja desmarcar todos os itens? Os valores reais serão limpos, mas os itens continuam na lista.")) return
    ...
    try {
        await Promise.all(items.map(item => 
            shoppingService.updateItem(item.id, { is_purchased: false, actual_price: null })
        ))
        toast.success("Lista resetada para nova compra!")
    } catch (error) {
        toast.error("Erro ao resetar alguns itens.")
    }
  }

  // 🗑️ EXCLUIR CUPOM ESPECÍFICO
  const handleDeleteReceipt = async (receiptId: string) => {
    if (!window.confirm("Deseja remover este cupom do arquivo?")) return
    
    try {
        await shoppingService.deleteReceipt(receiptId) // Certifique-se que essa função existe no seu shoppingService
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-4 md:p-8 animate-in fade-in duration-500 pb-32 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
        ...
        <button onClick={() => fileInputRef.current?.click()} disabled={isScanning} className="group flex items-center gap-3 bg-white/5 border border-white/10 hover:bg-white/10 text-white px-6 py-3.5 rounded-2xl text-xs font-black uppercase tracking-widest transition-all hover:scale-105 shadow-xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
              ...
              <div className="bg-[#09090b] border border-white/5 rounded-3xl p-6 shadow-2xl">
                      ...
                      {receipts.map((receipt) => (
                          <div key={receipt.id} className="flex justify-between items-center bg-white/[0.03] p-3 rounded-xl hover:bg-white/[0.06] transition-colors group">
                              ...
                              <div className="flex items-center gap-1">
                                <button onClick={() => window.open(receipt.image_url, '_blank')} className="p-2 text-gray-400 hover:text-indigo-400 transition-colors" title="Download"><Download size={14} /></button>
                  ...
                  <button onClick={() => !isScanning && fileInputRef.current?.click()} disabled={isScanning} className="mt-4 w-full bg-white/5 hover:bg-white/10 text-white px-4 py-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-all flex justify-center items-center gap-2">
                      {isScanning ? <Loader2 size={14} className="animate-spin text-indigo-400"/> : <Camera size={14} className="text-indigo-400"/>}
                      {isScanning ? 'PROCESSANDO...' : 'ADICIONAR CUPOM'}
                  </button>
              </div>
          </div>
      </div>
