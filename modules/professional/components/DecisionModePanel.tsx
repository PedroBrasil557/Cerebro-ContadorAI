...
export default function DecisionModePanel() {
  ...
  useEffect(() => {
    void businessFinanceService.getDashboardData().then((data) => {
      ...
      const gross = sales.reduce((sum, item) => sum + Number(item.amount), 0)
      const observedExpenses = data.transactions.filter((item) => item.type === 'despesa_fixa' || item.type === 'despesa_variavel').reduce((sum, item) => sum + Math.abs(Number(item.amount)), 0)
      setRevenue(gross)
      setExpenses(observedExpenses)
      setAverageSale(sales.length ? gross / sales.length : 0)
      setVariableCost(data.costs.reduce((sum, item) => sum + Number(item.cost_per_use || 0), 0))
